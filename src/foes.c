...
static void	play_foe(t_game *game, t_foe *foe, t_pos player, t_offset offset)
{
	...
	if (distance.y && (abs(distance.y) < abs(distance.x)))
		move_foe(game, foe, offset_direction((t_offset){.x = 0, .y = distance.y}
				), offset_direction((t_offset){.x = distance.x, .y = 0}));
	else
		move_foe(game, foe, offset_direction((t_offset){.x = distance.x, .y = 0}
				), offset_direction((t_offset){.x = 0, .y = distance.y}));
}
